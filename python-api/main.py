...
def solve_dynamic_problem(data: ProblemDataModel): # 引数でデータを受け取る
    ...
    # 1. 計算対象のデータを抽出
    category_key = data.targetObjectCategoryKey
    ...
    if pulp.LpStatus[prob.status] == "Optimal":
        for r in route_keys:
            ...
            if amount > 0:
                results.append({"supplyNode": r[0], "demandNode": r[1], "amount": amount})
        return {"status": "Optimal", "totalCost": pulp.value(prob.objective), "routes": results}
    else:
        return {"status": pulp.LpStatus[prob.status], "totalCost": None, "routes": []}
